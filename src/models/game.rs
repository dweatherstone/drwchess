...
impl Game<'_> {
    pub fn new<'a>(renderer: &'a TextureCreator<WindowContext>) -> Game<'a> {
        ...
        let possible_moves = generator.generate_moves(&board, player);
    ...
    fn draw_last_move(&self, canvas: &mut WindowCanvas, width: i32, height: i32) {
        match self.last_move {
            ...
            Some(m) => {
                ...
                canvas.fill_rect(Rect::new((m.start % self.board.size) as i32 * case_width,
                                           (m.start / self.board.size) as i32 * case_height,
                                           case_width as u32,
                                           case_height as u32));
                canvas.set_draw_color(Color::RGBA(255, 255, 0, 30));
                canvas.fill_rect(Rect::new((m.end % self.board.size) as i32 * case_width,
    ...
    fn update_new_moves(&mut self) {
        self.possible_moves = self.move_generator.generate_moves(&self.board, self.current_player);
    }
